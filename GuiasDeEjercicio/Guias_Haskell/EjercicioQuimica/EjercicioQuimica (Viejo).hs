--SENCILLAS: SE LLAMAN ELEMENTO Y TIENEN
--Nombre, Simbolo quimico, Numero atomico

--COMPUESTAS: Se llaman compuestos y tienen una serie de componentes
--Un componente es un par formado por sustancia y cant de moleculas (sustancia, cantidadMoleculas). La sustancia del componente puede ser elemento o compuesto

--Compuestos tienen:
--Nombre y simbolo o formula quimica (la cual no nos interesa saber en todo momento)

--TODAS LAS SUTANCIAS TIENEN UN GRUPO O ESPECIE Y SON:
-- Metal / No Metal / Halogeno / Gas noble


data Elemento = Elemento {
    nombreElemento :: String,
    simboloQuimicoE :: String,
    numeroAtomico :: Int,
    grupoElemento :: Especie
} deriving (Show)

data Compuesto = Compuesto {
    nombreCompuesto :: String,
    grupoCompuesto :: Especie,
    componentes :: [(TipoSustancia, Int)]
} deriving (Show)

data Especie = Metal | NoMetal | Halogeno | GasNoble deriving(Eq, Show)

data TipoSustancia = Simple Elemento | Compuesta Compuesto deriving (Show)

--1)Modelar las siguientes sustancias:
--El hidrógeno y el oxígeno
--El agua, sustancia compuesta por 2 hidrógenos y 1 un oxígeno.

hidrogeno :: TipoSustancia
hidrogeno = Simple Elemento {
    nombreElemento = "Hidrogeno",
    simboloQuimicoE = "H",
    numeroAtomico = 1,
    grupoElemento = NoMetal
}

oxigeno :: TipoSustancia
oxigeno = Simple Elemento {
    nombreElemento = "Oxigeno",
    simboloQuimicoE = "O",
    numeroAtomico = 16,
    grupoElemento = NoMetal
}

fluor :: TipoSustancia
fluor = Simple Elemento {
    nombreElemento = "Fluor",
    simboloQuimicoE = "F",
    numeroAtomico = 17,
    grupoElemento = Halogeno
}

mercurio :: TipoSustancia
mercurio = Simple Elemento {
    nombreElemento = "Mercurio",
    simboloQuimicoE = "Hg",
    numeroAtomico = 12,
    grupoElemento = Metal
}

agua :: TipoSustancia
agua = Compuesta Compuesto {
    nombreCompuesto = "Agua",
    grupoCompuesto = NoMetal,
    componentes = [(hidrogeno, 2), (oxigeno,1)]
} 

--2) Saber si conduce bien una sustancia.
--El criterio actual es electricidad y calor



conduceBien :: TipoSustancia -> Bool
conduceBien (Simple (Elemento _ _ _ grupoElemento)) = grupoElemento == Metal
conduceBien (Compuesta (Compuesto _  _ listaSustancias)) = all ((Metal==) . grupoSustancia . fst) listaSustancias

grupoSustancia :: TipoSustancia -> Especie
grupoSustancia (Simple(Elemento _ _ _ grupoElemento)) = grupoElemento
grupoSustancia (Compuesta(Compuesto _  grupoCompuesto _)) = grupoCompuesto


--3) Obtener el nombre de unión de un nombre. Esto se logra añadiendo “uro” al final del nombre, pero solo si el nombre termina en consonante. Si termina en vocal, se busca hasta la última consonante y luego sí, se le concatena “uro”. Por ejemplo, el nombre de unión del Fluor es “fluoruro”, mientras que el nombre de unión del mercurio es “mercururo

esVocal letra = elem letra "aeiouAEIOU"


unionNombreSustancia :: TipoSustancia -> String
unionNombreSustancia(Simple (Elemento nombreElemento _ _ _))
    | (not.esVocal.last) nombreElemento = nombreElemento ++ "uro"
    | otherwise                         = listaParaConcatenar nombreElemento ++ "uro"


unionNombreSustancia(Compuesta(Compuesto nombreSustancia  _ _))
    | (not.esVocal.last) nombreSustancia = nombreSustancia ++ "uro"
    | otherwise                          = listaParaConcatenar nombreSustancia ++ "uro"




ultimasVocales :: String -> String
ultimasVocales lista = takeWhile esVocal $ reverse lista

listaParaConcatenar :: String -> String
listaParaConcatenar lista = take (length lista - length (ultimasVocales lista)) lista


--4) Combinar 2 nombres. Al nombre de unión del primero lo concatenamos con el segundo, agregando un “ de “ entre medio. Por ejemplo, si combino “cloro” y “sodio” debería obtener “cloruro de sodio"


nombre (Simple (Elemento nombreElemento _ _ _)) = nombreElemento
nombre (Compuesta (Compuesto nombreSustancia  _ _)) = nombreSustancia


combinarNombres :: TipoSustancia -> TipoSustancia -> String
combinarNombres sustancia1 sustancia2 = unionNombreSustancia sustancia1 ++ " de " ++ nombre sustancia2

--5)Mezclar una serie de componentes entre sí. El resultado de dicha mezcla será un compuesto. Sus componentes serán los componentes mezclados. El nombre se forma de combinar los nombres de la sustancia de cada componente. La especie será, arbitrariamente, un no metal.

nombre' = map nombre 
simbolo' = map simbolo

simbolo (Simple (Elemento _ simboloQuimicoE _ _)) = simboloQuimicoE




mezclarComponentes :: [TipoSustancia] -> TipoSustancia 
mezclarComponentes listaComponentesMezcla = Compuesta Compuesto {
    nombreCompuesto = foldl1 (++) (nombre' listaComponentesMezcla),
    grupoCompuesto = NoMetal,
    componentes = [(hidrogeno, 0)] --falta resolver esto
 }
